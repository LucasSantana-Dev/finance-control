...
@ActiveProfiles("test")
@Transactional
class TransactionCategoryServiceTestContainersTest {
    ...
    void create_WithSpecialCharacters_ShouldHandleCorrectly() {
        // Given
        TransactionCategoryDTO createDTO = new TransactionCategoryDTO();
        createDTO.setName("Category with Special Characters: @#$%^&*()");
        ...
        // Then
        assertThat(result).isNotNull();
        assertThat(result.getName()).isEqualTo("Category with Special Characters: @#$%^&*()");

        // Verify it was saved to database
        TransactionCategory savedCategory = transactionCategoryRepository.findById(result.getId()).orElse(null);
        assertThat(savedCategory).isNotNull();
        assertThat(savedCategory.getName()).isEqualTo("Category with Special Characters: @#$%^&*()");
    }
    ...
    void create_WithUnicodeCharacters_ShouldHandleCorrectly() {
        // Given
        TransactionCategoryDTO createDTO = new TransactionCategoryDTO();
        createDTO.setName("Category with Unicode: 中文 العربية русский");
        ...
        // Then
        assertThat(result).isNotNull();
        assertThat(result.getName()).isEqualTo("Category with Unicode: 中文 العربية русский");

        // Verify it was saved to database
        TransactionCategory savedCategory = transactionCategoryRepository.findById(result.getId()).orElse(null);
        assertThat(savedCategory).isNotNull();
        assertThat(savedCategory.getName()).isEqualTo("Category with Unicode: 中文 العربية русский");
    }
}
